...
void act_slap();
void act_life_m(char *f_loc,int clear);
...
{
	char re[]={"?!@#$%^&*+-/~<>(){}[]×◎○┌┐┘└┬┤┴├┼─│↑↓←→█？"};//20
	int shape_life[25][25]={0},shape_life_n[25][25]={0};
	FILE *fp;
	int a,i,j,x,y,n,cont;
	char a_c;
	...
	for(n=1;n<=50;n++)
	{
		for(i=0;i<=24;i++)
		{
			for(j=0;j<=24;j++)
			{
				shape_life_n[i][j]=shape_life[i][j];
			}
		}
		for(i=1;i<=25;i++)
		{
			...
			{
				cont=0;
				if(j-1>=1&&j-1<=25&&i>=1&&i<=25)
				{
					if(shape_life_n[i-1][j-2]==1)
					{
						cont++;
					}
				}
				if(j+1>=1&&j+1<=25&&i>=1&&i<=25)
				{
					if(shape_life_n[i-1][j]==1)
					{
						cont++;
					}
				}
				if(j>=1&&j<=25&&i-1>=1&&i-1<=25)
				{
					if(shape_life_n[i-2][j-1]==1)
					{
						cont++;
					}
				}
				if(j>=1&&j<=25&&i+1>=1&&i+1<=25)
				{
					if(shape_life_n[i][j-1]==1)
					{
						cont++;
					}
				}
				if(j-1>=1&&j-1<=25&&i-1>=1&&i-1<=25)
				{
					if(shape_life_n[i-2][j-2]==1)
					{
						cont++;
					}
				}
				...
				{
					shape_life[i-1][j-1]=0;
				}
				
				if(shape_life[i-1][j-1]==1)
				{
					gotoxy(j*2+1,i+1);
					switch(random(1,11))
		        	{
...
int stage(char *stage_loc)
{
	...
	while(1)
	{
		...
		switch(read_mode)
		{
			...
			case 7:
			{
				act_life_m(read_path,1);
				break;
			}
			case 8:
			{
				act_life_m(read_path,0);
				break;
			}
			default:
			{
